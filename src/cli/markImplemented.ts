...
function fieldLineRegex(key: string): RegExp {
 ...
 *  quotes, then blank-after-trim collapses to "absent") — used here only to classify the
 *  *current* implementedIn value for the idempotent/refusal guard, matching what the parser
 *  would have reported for the same text. */
function normalizeFieldValue(raw: string | undefined): string | null {
 ...
 *  a rewrite can't afford the line-ending normalization a split/join would silently perform.
 *  `null` when there's no well-formed leading `---`/`---` fence, mirroring sliceDoc.ts's own
 *  "unterminated fence = no frontmatter" treatment. */
function locateFrontmatterInner(markdown: string): { innerStart: number; innerEnd: number } | null {
