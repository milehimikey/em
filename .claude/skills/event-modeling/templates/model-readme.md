...
never reaches a read model directly — the event goes between them. Every event a command records
is read by some read model, so each State Change slice is paired with the State View slice that
projects its event. A read model repeated along the timeline (`view X again`) shows the same
projection at a later point; the instances are never connected to one another.
