...
3. Render the slice's own diagram snippet: `em render <model>.em --slice "<slice name>" -o
   slices/<slice-name>.svg` (kebab-case, matching the doc's filename), and add
   `![Diagram](./<slice-name>.svg)` near the top of the doc, right after the metadata bullet list.
...
em render <name>.em --slice "<slice-name>" -o slices/<slug>.svg   # this slice's own snippet
