...
the spec (read-only, except the merge-time `status`/`implementedIn` flip); implement exactly one
slice doc per PR — a `covers:` doc's two keys ship together in that one PR, an `again` view
